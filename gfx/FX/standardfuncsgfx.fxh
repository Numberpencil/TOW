...
PixelShader = 
{
	...
	float4 checkForChange(float4 inColour)
	{
		...
		float3 currentRGB = inColour;
		
		float tolerance = 0.01; 
		
		float3 targetChangeColorGER = float3(0.0125, 0.0125, 0.0125); 
		float3 targetChangeColorITA = float3(0.02776, 0.1166, 0.04614); 

		
		float3 realGER       = float3(0.0125, 0.0125, 0.0125); // GER (42 42 42)
		float3 realGEN       = float3(0.0335, 0.0335, 0.0335); // GEN (64 64 64)
		float3 realRKU       = float3(0.0463, 0.0298, 0.0102); // RKU (71 48 13)
		float3 realRKO       = float3(0.1062, 0.1062, 0.1062); // RKO (115 115 115)
		float3 realRKG       = float3(0.0152, 0.0116, 0.0232); // RKG (29 39 52)
		float3 realITA       = float3(0.0278, 0.1166, 0.0461); // ITA (23 120 51)
		float3 realALB       = float3(0.0400, 0.0846, 0.0511); // ALB (53 104 68)
		
		if (distance(currentRGB.rgb, realGER.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorGER.rgb, realGER.rgb, getZoom());
		}
		else if (distance(currentRGB.rgb, realGEN.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorGER.rgb, realGEN.rgb, getZoom());
		}
		else if (distance(currentRGB.rgb, realRKU.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorGER.rgb, realRKU.rgb, getZoom());
		}
		else if (distance(currentRGB.rgb, realRKO.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorGER.rgb, realRKO.rgb, getZoom());
		}
		else if (distance(currentRGB.rgb, realRKG.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorGER.rgb, realRKG.rgb, getZoom());
		}

		else if (distance(currentRGB.rgb, realITA.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorITA.rgb, realITA.rgb, getZoom());
		}
		else if (distance(currentRGB.rgb, realALB.rgb) < tolerance)
		{
			inColour.rgb = lerp(targetChangeColorITA.rgb, realALB.rgb, getZoom());
		}
